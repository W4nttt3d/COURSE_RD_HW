...
{
public:
	HalvesMonsterLevel(Tribe tribe) : m_targetTribe(tribe) {}

	virtual void apply(Munchkin*, Monster* monster) override
	{
		if (monster->getTribe() == m_targetTribe)
		{
	...
	virtual std::string getFullInfo() const override
	{
		std::string tribeString;
		switch (m_targetTribe)
		{
		case Tribe::Human: 
			tribeString = "Human"; 
			break;
		...
		return "Halves the power of " + tribeString + " monsters";
	}
